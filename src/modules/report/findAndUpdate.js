...
const findAndUpdate = (bot, msg, username, b_date, message, ach, parse) => {
  ...
  Warrior.findOne({ t_id: userId }).then(res => {
    if (res) {
      ...
      const isOld = ~res.battles.findIndex(
        v => v.date.getTime() === b_date.getTime()
      );
      if (!isName) {
        bot.sendMessage(chatId, 'Это не твой репорт. Не обманывай.', {
